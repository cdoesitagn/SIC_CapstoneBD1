...
# Tạo file log cho consumer realtime
...
# Khai báo thread, mỗi thread tương ứng với một consumer
def consumer_thread(thread_id, topic):
    consumer = Consumer(kafkaConfigs)
    # Mỗi consumer tương ứng với topic đăng ký
    consumer.subscribe([topic])
    try: 
        while (datetime.datetime.now().time() > start_time and datetime.datetime.now().time() < end_time):
            # Consumer lấy data từ topic
            message = consumer.poll(1.0)
            # Kiểm tra có lỗi đối với dữ liệu hay không, nếu không chuyển đổi dữ liệu, chèn Id vào các mã sau đó truyền vào collection realtimePrices trong cơ sở dữ liệu
            if message is None:
                continue
            if message.error():
